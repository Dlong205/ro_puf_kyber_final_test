...
#   constraints/ro_physical_lock_allpairs_zynq7020.expected
...
set checkpoint [file join $root_dir build puf_allpairs_characterization \
...
set output_xdc [file join $root_dir constraints \
    ro_physical_lock_allpairs_zynq7020.xdc]
...
set anchor_file [file join $root_dir constraints \
    ro_physical_lock_allpairs_zynq7020.expected]
...
if {![file exists $checkpoint]} {
    error "Routed all-pairs checkpoint not found: $checkpoint"
}
...
if {[file exists $anchor_file]} {
    ...
    puts $anchor_channel [format {# Bootstrapped %s by export_ro_physical_lock_allpairs.tcl} \
        [clock format [clock seconds] -gmt true -format "%Y-%m-%d %H:%M:%S UTC"]]
    close $anchor_channel
    puts "PUF_ALLPAIRS_LOCK_BOOTSTRAP=1"
}
...
puts $channel "## Complete RO physical lock for the all-pairs characterization image."
...
puts "PUF_ALLPAIRS_LOCK_ENDPOINT_CELL_COUNT=[llength $endpoint_cells]"
puts "PUF_ALLPAIRS_LOCK_NET_COUNT=[llength $ro_nets]"
puts "PUF_ALLPAIRS_LOCK_XDC=$output_xdc"
puts "PUF_ALLPAIRS_LOCK_FINGERPRINT=$output_fingerprint"
puts "PUF_ALLPAIRS_LOCK_DCP_SHA=$checkpoint_sha256"
close_design