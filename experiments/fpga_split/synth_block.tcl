...
set required_bus [dict get [dict create client K server K kdf seed_out \
    fe key_out puf response] $block]
set observable [get_ports -quiet "${required_bus}*"]
set required_width [dict get [dict create client 256 server 256 kdf 512 \
    fe 192 puf 264] $block]
...
puts $fd "k\t[expr {$block in {client server} ? 2 : {not_applicable}}]"
