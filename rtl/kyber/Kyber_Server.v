...
always @(posedge clk) begin
    if(rst) begin
        ...
        state <= next_state;
        // $display("Kyber_Server: st=%h->%h, kc=%h, o1full=%b o1empty=%b, o0full=%b o0empty=%b, ntt.st=%h, ntt.i=%h, ntt.j=%h, ntt.k=%h, ntt.NTT=%h, patt72=%b eta3_72=%b oena=%b ac=%h",
        //    state, next_state, keccak_ctr, ofifo1_full, ofifo1_empty, ofifo0_full, ofifo0_empty, ntt.state, ntt.ctr_i, ntt.ctr_j, ntt.ctr_k, ntt.ctr_NTT,
        //    patt_r[72], eta3_r[72], ofifo_ena, absorb_ctr);
        ...
        // Generate pulse when entering state 11
...
always @(posedge clk) begin
	if(rst)
		...
		CCA_enc <= 1'h 0;
	else if(state == 6'h 2d) begin
		CCA_enc <= 1'h 1;
		$display("SERVER RECOVERED m': %h", m);
	end
	else
		CCA_enc <= CCA_enc;
end
...
reg valid_out_reg;
always @(posedge clk) begin
    if (rst) valid_out_reg <= 1'b0;
    else valid_out_reg <= OFIFO_req;
end
...
always @(posedge clk) begin
	if(start)
		...
		$display("CMP: cmp0=%h cmp1=%h equal_so_far=%b", cmp0, cmp1, cmp0 == cmp1 ? equal : 1'h0);
	end
	else
		equal <= equal;
end
