## Zynq-7020 constraints for the Phase B PUF64 single-RO microbenchmark.
## Target: xc7z020clg400-2

## Physical 50 MHz board oscillator at N18 -> real 100 MHz system clock.
...
set_property ALLOW_COMBINATORIAL_LOOPS true [get_nets -quiet -hierarchical -filter {NAME =~ "*u_bench*ro0*/t*"}]
set_false_path -through [get_nets -quiet -hierarchical -filter {NAME =~ "*u_bench*ro0*/t*"}]
set_property DONT_TOUCH true [get_cells -quiet -hierarchical -filter {NAME =~ "*u_bench*ro0*/u_backend/LUT6_*"}]
